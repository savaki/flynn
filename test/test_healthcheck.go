...
var _ = c.Suite(&HealthcheckSuite{})
...
func (s *HealthcheckSuite) TestWithoutChecker(t *c.C) {
	...
	_, err := s.discoverdClient(t).Instances("ping-without-checker", 10*time.Second)
	t.Assert(err, c.IsNil)
}
...
func (s *HealthcheckSuite) TestFailure(t *c.C) {
	...
	_, err := s.discoverdClient(t).Instances("healthcheck-failure", 10*time.Second)
...
func (s *HealthcheckSuite) TestKilldown(t *c.C) {
	...
	app, _ := s.createAppWithService(t, "printer", &host.Service{
		...
		Create: true,
		Check:  &host.HealthCheck{Type: "tcp", KillDown: true},
	})
